...
//' @export
// [[Rcpp::export]]
List SVP_new(std::vector<double> data,
         double gamma,
         std::string test,
         bool prune_if_unvalid = true)
{
  ...
  if (prune_if_unvalid == true)
  {
    for (size_t t = 1; t < n + 1; ++t)
    {
      ...
      // iterate through candidate changepoints for current `t` (inner loop).
      // Note: `continue` inside this loop will start the next `k` iteration (next candidate `s`) and
      // will NOT advance the outer `t` loop. To skip to the next `t`, we set a flag and break the inner loop.
      bool skip_to_next_t = false;
      ...
      for (size_t k = 0; k < INDEX.size(); ++k)
      {
        ...
        if (invalid_update && R(s, 1) + 1 == best_K) {
          ...
          // print catching up for debugging
            Rcout << "Catching up updates for t: " << t << ", s: " << s << std::endl;
        ...
        if (valid == true)
        {
          ...
          Rcout << "t: " << t << ", s: " << s << " is invalid." << std::endl;
